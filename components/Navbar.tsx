    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
      ...
      <nav className={`mx-auto flex max-w-7xl items-center justify-between px-4 lg:px-8 transition-all duration-500 ${
        ...
        <div className="hidden items-center gap-0.5 md:flex">
          {NAV_ITEMS.map((item) => (
            <Link key={item.href} href={item.href}
              className={`relative rounded-lg px-3 py-2 text-base font-medium transition-all duration-300
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* Cart */}
          <Link href="/cart" aria-label={cartCount > 0 ? `장바구니 ${cartCount}개` : '장바구니'} className="relative rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-ocean-600">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5 w-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
            </svg>
            {cartCount > 0 && (
              <span role="status" aria-live="polite" className="absolute -right-0.5 -top-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-coral-500 font-montserrat text-[10px] font-bold text-white">
                {cartCount}
              </span>
            )}
          </Link>
